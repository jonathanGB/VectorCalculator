...
$(document).ready(function() {
    ...
    $('#enlever a').on('click', function() {
    	...
    	if (nombre_retire > 1)
        	$('#btn2 button').eq(0).html('Remove ' + nombre_retire +' vectors');
        else if (nombre_retire == 1)
        	$('#btn2 button').eq(0).html('Remove ' + nombre_retire +' vector');
    });
    ...
    $('#btn1 button:eq(0)').on('click', function() {   	
    	...
    	if(nombre_vecteurs > 7)
    	{
    		...
    		$(this).popover({
    			container: 'body',
    			content: 'Maximum of 7 vectors at a time',
    			placement: 'top',
    		}).popover('show');
    	...
    	else
    	{
		    ...
		    else if (rad == true)	
		    {
		    	for(var i = 0; i < nombre_ajout; i++)
		    	{
		    		var indice = $('#mod tbody tr:last sub').html();
		    		if (rad == true)
		    			$('#mod tbody').append('<tr><td><span class="italic">v</span><sub>' + parseInt(parseInt(indice) + 1) + '</sub> </td><td> <input id="modu' + indice + '" type="text" maxlength="11" class="form-control"></td><td> <input id="orie' + indice + '" type="text" placeholder="radians" maxlength="6" class="form-control"><span> rad</span></td></tr>');
		    		else if (deg == true)
		    ...
		    else if (deg == true)
		    {
		    	for(var i = 0; i < nombre_ajout; i++)
		    	{
		    		var indice = $('#mod tbody tr:last sub').html();
		    		if (rad == true)
		    			$('#mod tbody').append('<tr><td><span class="italic">v</span><sub>' + parseInt(parseInt(indice) + 1) + '</sub> </td><td> <input id="modu' + indice + '" type="text" maxlength="11" class="form-control"></td><td> <input id="orie' + indice + '" type="text" placeholder="degrees" maxlength="6" class="form-control"><span> rad</span></td></tr>');
		    		else if (deg == true)
		    			$('#mod tbody').append('<tr><td><span class="italic">v</span><sub>' + parseInt(parseInt(indice) + 1) + '</sub> </td><td> <input id="modu' + indice + '" type="text" maxlength="11" class="form-control"></td><td> <input id="orie' + indice + '" type="text" placeholder="degrees" maxlength="6" class="form-control"><span> °</span></td></tr>');
		    		else
		    			$('#mod tbody').append('<tr><td><span class="italic">v</span><sub>' + parseInt(parseInt(indice) + 1) + '</sub> </td><td> <input id="modu' + indice + '" type="text" maxlength="11" class="form-control"></td><td> <input id="orie' + indice + '" type="text" placeholder="degrees" maxlength="6" class="form-control"><span></span></td></tr>');
    ...
    $('#btn2 button:eq(0)').on('click', function() {   	
    	...
    	if(nombre_vecteurs-nombre_retire <= 1)
    	{
    		$(this).popover({
    			container: 'body',
    			content: 'There must be a minimum of 2 vectors',
    			placement: 'top',
    		}).popover('show');
    ...
    $('#deg').click(function() {
        ...
        {
        	if ($(window).width() <= 768)
				$('#mod tbody tr:eq('+i+') td:eq(2) input').attr('placeholder', 'degrees');
			else
	...
	$('#radToDeg').click(function() {
		...
		$('#con p').html(' 1 radian = 1 <em>x</em> 180 / π')
	});
	$('#degToRad').click(function() {
		degToRad = true;
		radToDeg = false;

		$('#con p').html(' 1 degree = 1 <i>x</em> π / 180')
	});
	...
	$('#ajouterComp').click(function() {
		...
		else
		{
			$(this).popover({
				container: 'body',
				content: 'Maximum of 5 components',
				placement: 'top',
			}).popover('show');
	...
	$('#enleverComp').click(function() {
		...
		else
		{
			$(this).popover({
				container: 'body',
				content: 'Minimum of one component per vector',
				placement: 'top',
			}).popover('show');
	...
	$('#enleverComp2').click(function() {
		...
		else
		{
			$(this).popover({
				container: 'body',
				content: 'Minimum of two components',
				placement: 'top',
			}).popover('show');
	...
	$('#scalaire').click(function() {
		...
		$('#mul p:eq(0)').html('if <em>u = (u<sub>1</sub>, u<sub>2</sub>, ..., u<sub>n</sub>)</em> and <em>v = (v<sub>1</sub>, v<sub>2</sub>, ..., v<sub>n</sub>) ∈ ℛ <sup>n</sup> <br>u · v = (u<sub>1</sub>v<sub>1</sub> + u<sub>2</sub>v<sub>2</sub> + ... + u<sub>n</sub>v<sub>n</sub>) ∈ ℛ');
	});
	$('#vectoriel').on('click', function() {
		...
		$('#mul p:eq(0)').html('if <em>u = (u<sub>1</sub>, u<sub>2</sub>, u<sub>3</sub>)</em> and <em>v = (v<sub>1</sub>, v<sub>2</sub>, v<sub>3</sub>) ∈ ℛ <sup>3</sup><br>u ∧ v = (u<sub>2</sub>v<sub>3</sub> - u<sub>3</sub>v<sub>2</sub> , u<sub>3</sub>v<sub>1</sub> - u<sub>1</sub>v<sub>3</sub> , u<sub>1</sub>v<sub>2</sub> - u<sub>2</sub>v<sub>1</sub>) ∈ ℛ <sup>3</sup>');
	...
	$('#mixte').click(function() {
		...
		$('#mul p:eq(0)').html('if <em>u = (u<sub>1</sub>, u<sub>2</sub>, u<sub>3</sub>)</em> , <em>v = (v<sub>1</sub>, v<sub>2</sub>, v<sub>3</sub>)</em> and <em>w = (w<sub>1</sub>, w<sub>2</sub>, w<sub>3</sub>) ∈ ℛ <sup>3</sup><br>u · (v ∧ w) = (u<sub>1</sub>(v<sub>2</sub>w<sub>3</sub> - v<sub>3</sub>w<sub>2</sub>) + u<sub>2</sub>(v<sub>3</sub>w<sub>1</sub> - v<sub>1</sub>w<sub>3</sub>) + u<sub>3</sub>(v<sub>1</sub>w<sub>2</sub> - v<sub>2</sub>w<sub>1</sub>))∈ ℛ ');
	
		if($(window).width() <= 400 && mixte == true)
			$('#mul p:eq(0)').html('<em>u</em>, <em>v</em> and <em>w ∈ ℛ <sup>3</sup> | u · (v ∧ w) ∈ ℛ</em>');
	});
	...
	$('#polaire').click(function() {
		...
		if ($('#com p').html() != '<!-- Inserer reponse -->')
			$('#com p').html('<b>norm of the resultant:</b><em> ' + resultante + '</em> <b><br> orientation:</b><em> ' + orientationDeg + '</em><b>° or </b><em>' + orientationRad + '</em> rad').addClass('reponseEsthetique');
		else if ($('#mod p').html() != '<!-- Inserer reponse -->')	
			$('#mod p').html('<b>norm of the resultant:</b><em> ' + resultante + '</em> <b><br> orientation:</b><em> ' + orientationDeg + '</em><b>° or </b><em>' + orientationRad + '</em> rad').addClass('reponseEsthetique');	
	});
	$('#composantes').click(function() {
		...
		if ($('#com p').html() != '<!-- Inserer reponse -->')
			$('#com p').html('<b>com. x of the resultant:</b><em> ' + comX_total + '</em> <b><br> com. y of the resultant:</b><em> ' + comY_total + '</em>').addClass('reponseEsthetique');
		else if ($('#mod p').html() != '<!-- Inserer reponse -->')
			$('#mod p').html('<b>com. x of the resultant:</b><em> ' + comX_total + '</em> <b><br> com. y of the resultant:</b><em> ' + comY_total + '</em>').addClass('reponseEsthetique');
	});
	...
	$('#btn3').click(function() {
		...
		else
		{
			$(this).popover({
				container: 'body',
				content: 'There is a problem with your input',
				placement: 'top',
			}).popover('show');
	...
	$('#angleConverti button').click(function() {
		...
		if (radToDeg == false && degToRad == false)
		{
			$(this).popover({
				container: 'body',
				content: 'You must choose a type of conversion',
				placement: 'bottom',
			}).popover('show');
		...
		else if (radToDeg == true)
		{
			...
			if (!isNaN(entree) && isFinite(entree) && /^-?[0-9]+(\/)?[0-9.]*$/.test(entree)  && !/\.$/.test(entree))
			{
				...
				if (conversion_modulo == 'vide')
					...
					$('#reponseConversion').val(conversion + '° or ' + conversion_modulo + '°')
	...
	$('#afficherResultat').click(function() {
		...
		for (var i = 0; i < nombre_composantes_unitaire; i++)
			...
			if (count == nombre_composantes_unitaire)
			{
				...
				if (nombre_composantes_unitaire == 2)
					$('#uni p:eq(1)').html('<b>Unit direction vector is:</b> (' + unitaires_resultat[0] + ' , ' + unitaires_resultat[1] + ')');
				else if (nombre_composantes_unitaire == 3)
					$('#uni p:eq(1)').html('<b>Unit direction vector is:</b> (' + unitaires_resultat[0] + ' , ' + unitaires_resultat[1] + ' , ' + unitaires_resultat[2] + ')');
				else if (nombre_composantes_unitaire == 4)
					$('#uni p:eq(1)').html('<b>Unit direction vector is:</b> (' + unitaires_resultat[0] + ' , ' + unitaires_resultat[1] + ' , ' + unitaires_resultat[2] + ' , ' + unitaires_resultat[3] + ')');
				else
					$('#uni p:eq(1)').html('<b>Unit direction vector is:</b> (' + unitaires_resultat[0] + ' , ' + unitaires_resultat[1] + ' , ' + unitaires_resultat[2] + ' , ' + unitaires_resultat[3] + ' , ' + unitaires_resultat[4] + ')');
			}
			else
			{
				$(this).popover({
	...
	$('#afficherProduit').click(function() {
		...
		if (scalaire == true)
		{
			...
			if (count == nombre_composantes)
			{
				...
				$('#mul p:eq(1)').html('<b>Dot product is:</b> ' + resultat);
			}
			else
			{
				$(this).popover({
		...
		else if (vectoriel == true)
		{
			...
			if (count == 3)
			{
				...
				$('#mul p:eq(2)').html('<b>Cross product is:</b> (' + resultatX + ' , ' + resultatY + ' , ' + resultatZ + ')');
			}
			else
			{
				$(this).popover({
		...
		else if (mixte == true)
		{
			...
			if (count == 3)
			{
				...
				$('#mul p:eq(3)').html('<b>Triple product is:</b> ' + resultat);
			}
			else
			{
				$(this).popover({
	...
	if ($(window).width() <= 768)
	{
		$('#mod tbody tr:eq(0) td:eq(2) input').attr('placeholder', 'degrees');
		$('#mod tbody tr:eq(1) td:eq(2) input').attr('placeholder', 'degrees');
	}
